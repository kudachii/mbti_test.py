...
def run_mbti_diagnostic():
    ...
    if 'scores' not in st.session_state:
        st.session_state.scores = {"E-I": 0, "S-N": 0, "T-F": 0, "J-P": 0}
    ...
    with st.form("mbti_form"):
        ...
        submit = st.form_submit_button("診断結果を表示する ✨")

    if submit:
        # スコア計算
        current_scores = {"E-I": 0, "S-N": 0, "T-F": 0, "J-P": 0}
        for i, (q_text, axis, weight) in enumerate(questions):
            current_scores[axis] += (user_answers[i] - 3) * weight

        # タイプ判定
        res = ("E" if current_scores["E-I"] >= 0 else "I") + \
        ...
        mbti_details = {
            ...
            "ISFP": {"name": "冒険家", "desc": "感性豊かで自由を愛する。自分らしく生きるアーティストです。", "mentor": "優しさに溢れるメンター (Default)"},
            ...
            "ESFP": {"name": "エンターテイナー", "desc": "明るく友好的。周囲を楽しませるムードメーカーです。", "mentor": "優しさに溢れるメンター (Default)"},
        ...
        detail = mbti_details.get(res)

        # 表示エリア
        st.divider()
        st.balloons()
        
        st.markdown(f"### 判定結果：{res}（{detail['name']}）")
        
        # 性格の特徴を表示
        st.info(f"💡 **あなたの特徴**\n\n{detail['desc']}")
        
        # おすすめメンターを表示
        st.success(f"📌 **おすすめメンター**\n\n**{detail['mentor']}**")
        
        # メンターからのひと言を表示
        st.write("---")
        st.markdown(f"💬 **メンターからのメッセージ**")
        st.markdown(f"*{mentor_quotes.get(detail['mentor'])}*")

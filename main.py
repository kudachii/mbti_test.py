...
def run_imperial_complete_app():
    # --- 1. 質問データ (32問：重み付けを追加) ---
    # 第3引数は「一致(5)」と答えた時に、その軸のプラス(1)かマイナス(-1)どちらに振るか
    questions = [
    ...
    mbti_db = {
        ...
        # 他の14タイプもここに同様の形式で追加することで、全タイプ判定可能になります
    }
    ...
    if not st.session_state["show_result"]:
        ...
        for i, (_, axis, weight) in enumerate(questions):
            ...
            # 重み(weight)を掛けることで、逆転質問に対応
            scores[axis] += (val - 3) * weight

        # 判定関数（0の場合はランダムに振り分け、ESTJへの偏りを防ぐ）
        def judge(val, positive, negative):
            if val > 0: return positive
            elif val < 0: return negative
            else: return random.choice([positive, negative])
        ...
        res = mbti_db.get(m_core, mbti_db.get("ESTJ")) # データ未登録時はESTJを返す
